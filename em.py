...
class EMGMM:
	...
	def run(self, n_components, n_init=1):
		...
		for digit in range(10):
			gmm = GaussianMixture(n_components=self.n_components[digit], n_init=n_init, random_state=0)
...
def main():
	...
	e.run(n_components=const_phonemes)
	print("Results for Clusters = Phonemes:\n")
	e.run(n_components=phonemes)
	print("Results for Clusters = Phonemes + Transitions:\n")
	e.run(n_components=phonemes_transitions)
